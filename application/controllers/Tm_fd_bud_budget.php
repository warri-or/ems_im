...
class Tm_fd_bud_budget extends Root_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Tm_fd_bud_budget');
        ...
        $this->controller_url='tm_fd_bud_budget';
    }
    ...
    private function system_forward($id)
    {
        if(isset($this->permissions['edit'])&&($this->permissions['edit']==1))
        {
            ...
            $data=array();
            $data['status_budget'] = $this->config->item('LABEL_FDR_FORWARDED');
            ...
            if ($this->db->trans_status() === TRUE)
            {
                ...
                $mail_data['message']='Field Day Budget forwarded to ZI';
                $mail_data['name']=$logged_in_user->name;
                $result=System_helper::send_email($mail_data);

                if($result['status']==true)
                {
                    $this->message='Budget Forwarded and '.$result['message'];
                    $this->system_list();
                }
                else
                {
                    $this->message='Budget Forwarded but '.$result['message'];
                    $this->system_list();
                }
                // mailing portion end
            }
