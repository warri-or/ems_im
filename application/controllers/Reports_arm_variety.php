...
class Reports_arm_variety extends Root_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Reports_arm_variety');
        $this->controller_url='reports_arm_variety';
    ...
    private function system_search()
    {
        ...
        {
            $data['title']="Arm Product Characteristics Report Search";
            $ajax['status']=true;
            $data['crops']=Query_helper::get_info($this->config->item('table_setup_classification_crops'),array('id value','name text'),array());
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("reports_arm_variety/search",$data,true));
            if($this->message)
            {
    ...
    private function system_list_variety()
    {
        ...
        $data['report']=$filters;
        $ajax['status']=true;
        $ajax['system_content'][]=array("id"=>"#variety_list_container","html"=>$this->load->view("reports_arm_variety/list_variety",$data,true));
    ...
    private function system_list()
    {
        ...
        if(isset($this->permissions['view'])&&($this->permissions['view']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>$this->load->view("reports_arm_variety/list",$data,true));
